...
body <- dashboardBody(tabItems(
  tabItem(tabName = "plays_over_t_tab",
          fluidRow(
            column(
              ...
              If you get \"Invalid Username or API Crash\" please try again.\
              If it continues to fail, try pausing music while data is being collected.",
              width = NULL)
              ),
            column(
              width = 6,
  ...
  tabItem(tabName = "artist_network",
          fluidRow(
            box(title = "Plots are Interactive!",
                width = 4,
                "Hover over nodes to see plays and tags. Click a node to highlight its neighbors."),
